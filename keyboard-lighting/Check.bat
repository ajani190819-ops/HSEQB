...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol='Tls12'; $b='%BASE%'; $d='%~dp0'; $n=0; foreach($f in 'Check.ps1','Aura-Background.ps1','Lighting-Panel.ps1','Lighting-Panel.bat','Update.ps1','Update.bat','Tray.ps1','Install.ps1','Install.bat','ui_controls.cs.txt'){ try{ Invoke-WebRequest \"$b/$f\" -OutFile (Join-Path $d $f) -UseBasicParsing -TimeoutSec 25; Unblock-File (Join-Path $d $f); $n++ } catch { Write-Host ('   could not download ' + $f) -ForegroundColor Yellow } }; Write-Host ('   got ' + $n + ' of 10 files') -ForegroundColor Gray"
